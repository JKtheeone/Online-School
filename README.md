...
Skillbox[3],основанная в 2016 году. На данный момент эта школа насчитывается более 300 тысяч пользователей, что говорит о высоком спросе на онлайн-образование.
...
В качестве другого примера можно привести онлайн-курсы Лекториума[4]. Лекториум — санкт-петербургский некоммерческий проект, занимающийся созданием учебных материалов в формате открытых онлайн-курсов, а также съёмкой и размещением видеолекций.Однако даже здесь есть платные курсы, хоть проект и обозначен как некоммерческий. А задача данного проекта реализовать полностью бесплатное обучение в онлайн формате.
...
На основании ER диаграммы создаем классы для каждой сущности.Ниже приведен пример создания класса Обучающийся(см. листинг 1,2).
...
Затем создаем хранилище , где будут храниться экземпляры классов(см. листинг 3,4).

Листинг 3 - хранилище для класса "Обучающийся".
~~~csharp
...
Далее создадим контроллер для каждой сущности и добавим набор web-методов, включая 4 базисные операции CRUD (create,delete,update,delete).Пример контроллера для класса "Обучающийся"(см. листинг 5)

Листинг 5 - пример контроллера.
...
Система выдает ошибку, где программа сообщает о том что заданного индекса не обнаружено в списке значений.
...
В рамках данного проекта была разработна информационная система для онлайн обучения. Для этого были последовательно выполнены следующие действия:


1.Определение внешних систем, которые могут повлиять на решение.

2.Определение целей,требований и условий  для системы.

3.Проверка полноты и достоверности выбранных данных.


В перспективе предполагается дальнейшее развитие данной системы, в виде установления связи с базой данных и добавления новых методов для работы с системой.
...
<a name="sourse_1">
 ...
 2.Рейтинг популярности платформ онлайн обучения. br-analytics.ru [Электронный ресурс]:Автор-Валерия Вебер, Издательство - Москва, Трёхпрудный пер., 9с1, дата публикации - 02.09.2021 - Режим доступа: https://br-analytics.ru/blog/top-20-education-2020-2021/ , свободный (дата обращения 15.12.2021)
 
 3.Skillbox. skillbox.ru [Электронный ресурс]. - Режим доступа: https://skillbox.ru/ , свободный (дата обращения 16.12.2021)
